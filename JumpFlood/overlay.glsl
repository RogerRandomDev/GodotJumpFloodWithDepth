...
void main() {
	...
	for (int x = -1; x <= 1; x++) {
		for (int y = -1; y <= 1; y++) {
			jfa_color = imageLoad(stencil_pass_image, uv + (ivec2(x, y) * 3)).rgb;
			if (jfa_color.r <= 0 && (jfa_color.g > 0 || jfa_color.b > 0)) {
				vec2 sample_closestPoint = round(jfa_color.gb * params.raster_size);
				float local_width = params.outline_width * (0.75f - (sin((sample_closestPoint.x * 0.15f) + params.time * 9.0f) + sin((sample_closestPoint.y * 0.15f) + params.time * 13.0f)) * 0.25f);
